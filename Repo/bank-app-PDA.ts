...
describe("bank-app", () => {
  ...
  it("Is deposited token!", async () => {
    let tokenMint = new PublicKey("BP9mQoqrZiLWSfBJ2di6y1qfGUUSW8N8gxVe1ZmnZeZm") //you should put your token mint here
  ...
  it("Withdraw token with pause logic!", async() => {
    ...
    let userReserveBefore = await program.account.userReserve.fetch(
      BANK_APP_ACCOUNTS.userReserve(provider.publicKey, mint)
    );
